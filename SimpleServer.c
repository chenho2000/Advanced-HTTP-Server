...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		// To make the server concurrency, fork when accept the connection, so server can serve multiple clients
		...
		if (child == 0)
		{
			...
			while (1)
			{
				...
				recv(curr_socket, buffer, BUF_SIZE, 0);
				...
				*/
				...
				free(res.res);
				bzero(buffer, sizeof(buffer));
				break;					//lost connection to child process once response is sent
			}
		}
		close(curr_socket);
	}
