		...
		if (child == 0)
		{
			...
			while (1)
			{
				...
				if (strlen(buffer) == 0)
				{
					...
					{ // efficiency issue, here decimal is 1.0, so at least 1s has to pass to break out the loop
						break;
					}
				}

				// To make the server pipelined, fork once get the request
				// Parent will get the request, childs response to client

				pid_t pip_child = fork();
